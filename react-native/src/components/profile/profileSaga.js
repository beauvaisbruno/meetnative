...
exports.profileSagas = [
    effects_1.takeEvery(profileReducer_1.ProfileActions.profileUpdate, updateSaga),
...
function* updateSaga(action) {
    try {
        ...
        profile.langsToLearn = LearnLevel_1.LearnLevel.sortKeysByValue(profile.langsToLearn);
        profile.langsToTeach = TeachLevel_1.TeachLevel.sortKeysByValue(profile.langsToTeach);
        profile.id = user.uid;
        const callOrTimeoutRes = yield sagaHelper_1.callOrTimeout(10000, firebase_1.firesaga.firestore.setDocument, "profiles/" + user.uid, profile, undefined);
        if (callOrTimeoutRes && callOrTimeoutRes.timeout)
            throw new Error("timeout");
        yield effects_1.put(reducerHelper_1.successAction(profileReducer_1.ProfileActions.profileUpdate));
        const prevProf = yield effects_1.select(sagaHelper_1.getUserProfile);
        const firstProfileUpload = prevProf === null;
        yield effects_1.put(reducerHelper_1.successAction(profileReducer_1.ProfileActions.profileLoad, profile));
        ...
        yield effects_1.put(reducerHelper_1.errorAction(profileReducer_1.ProfileActions.profileUpdate, error));
    }
...
function* logoutSaga() {
    try {
        yield effects_1.put(reducerHelper_1.fetchingAction(roomReducer_1.RoomActions.stopSyncMessages));
        ...
        yield effects_1.call(react_native_1.AsyncStorage.removeItem, "apollo-cache-persist");
        yield effects_1.call(apollo_1.client.resetStore);
        apollo_1.subscriptionClient.close(true);
        yield effects_1.put(reducerHelper_1.successAction(profileReducer_1.ProfileActions.profileLogout));
        yield effects_1.put(typesafe_actions_1.action(rootReducers_1.ClearAll));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(profileReducer_1.ProfileActions.profileLogout, error));
    }
}
...
function* googleLogout() {
    try {
        ...
        yield effects_1.call(react_native_google_signin_1.GoogleSignin.revokeAccess);
...
function* loadSaga() {
    try {
        ...
        if (user === null)
            throw Error("No user logged in");
        const snapshot = yield effects_1.call(firebase_1.firesaga.firestore.getDocument, "profiles/" + user.uid);
        if (snapshot && snapshot.data) {
            ...
            yield effects_1.put(reducerHelper_1.successAction(profileReducer_1.ProfileActions.profileLoad));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(profileReducer_1.ProfileActions.profileLoad, error));
    }
}
