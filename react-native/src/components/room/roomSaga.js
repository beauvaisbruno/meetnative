...
exports.roomSagas = [
    ...
    effects_1.takeEvery(roomReducer_1.RoomActions.stopSyncMessages, stopSyncMessagesSaga)
];
function* setMessageRead() {
    try {
        if (RootNavigation_1.getCurrentScreen() !== "RoomScreen")
            return;
        ...
        yield effects_1.call(firebase_1.firesaga.firestore.setDocument, "lastMessages/" + lastMessage.id, lastMessage, { merge: true });
        yield effects_1.put(reducerHelper_1.successAction(roomReducer_1.RoomActions.setMessageRead));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(roomReducer_1.RoomActions.setMessageRead, error));
    }
}
...
function* syncMessagesSaga() {
    try {
        ...
        yield effects_1.fork(syncMessage, "recipientId", onLoadRecipientSagaSuccess);
        yield effects_1.put(reducerHelper_1.successAction(roomReducer_1.RoomActions.syncMessages));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(roomReducer_1.RoomActions.syncMessages, error));
    }
}
...
function* syncMessage(whereClause, successActionCreator) {
    // @ts-ignore
    const channel = firebase_1.firesaga.firestore.channel(react_native_firebase_1.firestore()
        .collection("messages")
        .where(whereClause, "=", react_native_firebase_1.default.auth().currentUser.uid));
...
function onLoadRecipientSagaSuccess(messagesRes) {
    redux_1.store.dispatch(reducerHelper_1.fetchingAction(roomReducer_1.RoomActions.setMessageRead));
    return onLoadSagaSuccess(roomReducer_1.RoomActions.loadRecipient, messagesRes);
}
exports.onLoadRecipientSagaSuccess = onLoadRecipientSagaSuccess;
function onLoadSagaSuccess(action, messagesRes) {
    if (!messagesRes)
        return reducerHelper_1.successAction(roomReducer_1.RoomActions.syncMessages);
    const messages = [];
...
function onLoadSagaError(error) {
    return reducerHelper_1.errorAction(roomReducer_1.RoomActions.syncMessages, error);
}
exports.onLoadSagaError = onLoadSagaError;
function* sendMessageSaga(action) {
    try {
        const message = action.payload;
        const callOrTimeoutRes = yield sagaHelper_1.callOrTimeout(10000, firebase_1.firesaga.firestore.addDocument, "messages", message, undefined);
        ...
        yield effects_1.call(firebase_1.firesaga.firestore.setDocument, "lastMessages/" + message.authorId + "-" + message.recipientId, lastMessage, { merge: true });
        yield effects_1.put(reducerHelper_1.successAction(roomReducer_1.RoomActions.sendMessages));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(roomReducer_1.RoomActions.sendMessages, error));
    }
}
