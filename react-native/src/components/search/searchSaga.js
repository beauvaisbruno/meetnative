...
exports.searchSagas = [
    effects_1.takeEvery(searchReducer_1.SearchActions.searchProfiles, searchProfilesSaga)
];
function* searchProfilesSaga() {
    try {
        ...
        for (const threshold of thresholds) {
            const snapshot = yield effects_1.call(firebase_1.firesaga.firestore.getCollection, react_native_firebase_1.firestore()
                .collection("profiles")
                .where("geohash", ">", ngeohash.encode_int(lat - threshold, lon - threshold, 30))
                .where("geohash", "<", ngeohash.encode_int(lat + threshold, lon + threshold, 30)));
            ...
            snapshot.forEach((profile) => {
                ...
                if (utils_1.langMatch(userProfile.langsToLearn, otherProfile.langsToTeach) &&
        ...
        yield effects_1.put(reducerHelper_1.successAction(searchReducer_1.SearchActions.searchProfiles, profilesData));
    }
    catch (error) {
        console.log("error: ", error);
        yield effects_1.put(reducerHelper_1.errorAction(searchReducer_1.SearchActions.searchProfiles, error));
    }
}
