...
exports.logAsyncStorage = () => {
    react_native_1.AsyncStorage.getAllKeys((error, keys) => {
        if (error)
            console.log("getAllKeys error: ", error);
        if (keys)
            keys.forEach(key => {
                react_native_1.AsyncStorage.getItem(key).then(value => {
...
exports.createLangPickers = () => {
    ...
    langs.forEach(({ value, label }) => {
        rows.push(React.createElement(native_base_1.Picker.Item, { label: label, value: value, key: value }));
    });
    return rows;
};
...
function teachRows(profile, fontSize, userLearn, bestMatchOnly = false) {
    ...
    Object.keys(profile.langsToTeach).some((key) => {
        ...
        const matched = utils_1.langMatch(userLearn, { [key]: profile.langsToTeach[key] });
        if (matched)
            style.color = custom_1.default.brandSecondary;
        if (!bestMatchOnly || matched)
            teachRows.push(React.createElement(native_base_1.Text, { style: style, key: "teach-" + key }, Language_1.default.lang(key) +
                " " +
...
function learnRows(profile, fontSize, userTeach, bestMatchOnly = false) {
    ...
    Object.keys(profile.langsToLearn).some((key) => {
        const style = { fontSize, color: custom_1.default.textGreyLight };
        const matched = utils_1.langMatch({ [key]: profile.langsToLearn[key] }, userTeach);
        if (matched)
            style.color = custom_1.default.brandSecondary;
        if (!bestMatchOnly || matched)
            learnRows.push(React.createElement(native_base_1.Text, { style: style, key: "learn-" + key }, Language_1.default.lang(key) +
                " " +
...
exports.msgOnError = (touched, error) => {
    if (touched && error)
        return React.createElement(native_base_1.Text, { style: { color: custom_1.default.brandDanger } }, error);
    return undefined;
};
